...
module.exports = function(inputPath, outputPath, options, callback) {
			...
			return ensureEmptyDirectoryExists(outputTemplatesPath)
				.then(function() {
					return Promise.all([
						...
						copyFiles(themeAssetsPath, outputAssetsPath, { expand: shouldExpandSymlinks }),
						copyFiles(themeTemplatesPath, outputTemplatesPath, { expand: shouldExpandSymlinks }),
	...
	function savePreviewSiteAssets(previewFilesPath, themeAssetsPath, outputPath) {
		...
		function copyAssets(sourcePath, outputPath) {
			return copyFiles(sourcePath, outputPath, { expand: true });
		}
	...
	function copyFiles(sourcePath, outputPath, options) {
		options = options || {};
		var shouldExpandSymlinks = options.expand;
		return Promise.resolve(copy(sourcePath, outputPath, {
			expand: shouldExpandSymlinks
		}));
	}
